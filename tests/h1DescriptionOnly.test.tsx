...
describe('Description-only corrections preserve approved H1s',()=>{
  ...
  for(const [url] of Object.entries(details).filter(([url])=>!before.rows.some((r:{url:string})=>r.url===url)))it('identifies redirected model rather than inventing a page: '+url,()=>expect(redirects.redirects.some(r=>r.source===url)).toBe(true));
